...
for t in dirst
    ...
    try #try to load from xml files
        ...
        tbc = ThreeBodyTB.TB.read_tb_crys("projham_K.xml.gz", directory=tfull)
...
energy_tot, tbc, conv_flag = scf_energy(c, database=database_new)

println("We can compare our model energy $energy_tot with the DFT atomization energy ", dft.atomize_energy)
